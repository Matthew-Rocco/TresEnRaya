...
public class Cliente implements Runnable {
    ...
    //Variables necesarias para la conexion y comunicacion
    ...
    public Cliente(Main frame) {
        try {
            this.frame = frame;
            if(ipCliente == null){
                ipCliente = JOptionPane.showInputDialog(frame, "Ingrese la ip del host", "3 en Raya", 3);
            }
            this.frame.setIP(ipCliente);
            //Se cargan las imagenes de la X y O
            X = ImageIO.read(getClass().getResource("X.png"));
            O = ImageIO.read(getClass().getResource("O.png"));
            //Crea socket con el host y el puerto 
            cliente = new Socket(ipCliente, puerto);
            //Se declaran los streams de comunicacion
        ...
        try {
            ...
            while (true) {
                ...
                if (XO.equals(mensajes[3])) {
                    JOptionPane.showMessageDialog(frame, "GANASTEEEEE!", "Victoriaaaa!", 1);
                    new Main().setVisible(true);
                    frame.dispose();
                } else if ("EMPATE".equals(mensajes[3])) {
                    JOptionPane.showMessageDialog(frame, "EMPATE!", "u.u", 2);
                    new Main().setVisible(true);
                    frame.dispose();
                } else if (!"NADIE".equals(mensajes[3]) && !mensajes[3].equals(mensajes[0])) {
                    JOptionPane.showMessageDialog(frame, "PERDISTE BUUUUU!", "Malisimo u.u", 0);
                    new Main().setVisible(true);
                    frame.dispose();
                }
